...
function TemplateReferralCard({
  ...
  const fallbackDemo = demos.find((d) => d.id === theme.name);
  const imageUrl = theme.thumbnailUrl || fallbackDemo?.image || "";
  ...
  return (
    <motion.div
      ...
      className="group bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-md hover:border-rose-200/60 transition-all duration-300 overflow-hidden flex flex-col"
    >
      {/* Large Image Header */}
      <div className="relative h-48 w-full bg-slate-100 overflow-hidden">
        ...
        <div className="absolute bottom-3 left-3 right-3 z-10">
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            <span className="flex items-center gap-1 text-[10px] font-bold text-rose-50 bg-black/40 backdrop-blur-sm border border-white/10 px-2 py-0.5 rounded-full">
            ...
            <span className="flex items-center gap-1 text-[10px] font-medium text-slate-200 bg-black/40 backdrop-blur-sm border border-white/10 px-2 py-0.5 rounded-full">
              <Clock className="w-2.5 h-2.5" />
              {theme.durationDays}d access
            </span>
            {isInstant && (
              <span className="text-[10px] font-bold text-emerald-300 bg-black/40 backdrop-blur-sm border border-emerald-400/30 px-2 py-0.5 rounded-full">
                Instant
              </span>
            )}
          </div>
      ...
      {/* Card Body - Link & Share */}
      <div className="p-4 flex flex-col flex-1">
        {/* Link display */}
        <div className="flex items-center gap-2 bg-slate-50 border border-slate-100 rounded-xl px-3 py-2 group-hover:border-rose-200/60 transition-colors mb-4">
      ...
      {hasCode && themes.length > 0 && (
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {themes.filter(t => t.name !== "Romantic").map((theme, i) => (
