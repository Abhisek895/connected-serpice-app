    ...
    return (
      <div className="bg-rose-500/10 border border-rose-500/20 p-4 rounded-lg">
        <h3 className="text-rose-400 font-bold flex items-center gap-2">
          <ShieldAlert className="w-5 h-5" /> Error Loading System Health
        </h3>
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div>
        <h2 className="text-2xl font-bold text-white tracking-tight flex items-center gap-2">
          <Activity className="w-6 h-6 text-indigo-500" /> System Health
        </h2>
        <p className="text-slate-400 text-sm mt-1">Server metrics and database status.</p>
      </div>

      <div className="bg-[#111827] border border-slate-800 rounded-xl overflow-hidden shadow-sm">
        <div className="p-6 text-center text-slate-400 text-sm">
          {data.length === 0 ? "No data available for System Health." : "Loaded \ records."}
        </div>
      </div>
    </div>
  );
